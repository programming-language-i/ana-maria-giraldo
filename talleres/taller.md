...
R. Es concurrencia, ya que hay un solo núcleo alternando entre las tareas (entrelazado), no ejecutándose al mismo tiempo exacto.


2. Nombrar dos diferencias entre un proceso y un hilo *(memoria, costo, fallo)*.

R. Memoria: Los procesos tienen espacios de memoria independientes, los hilos comparten la misma memoria del proceso.<br>
Costo: Los procesos son más pesados de crear y consumen más recursos, los hilos son mas ligeros.<br>
Fallo: Si un proceso falla, no afecta a los demás, si un hilo no controlado rompe el proceso, fallan todos los hilos.<br>


3. ¿En qué estado del ciclo de vida está un hilo que ejecuta `time.sleep(2)`? ¿Qué devuelve `is_alive()`?

R. Con time.sleep(2), el hilo se detiene durante 2 segundos, is_alive() devuelve True porque aún no ha terminado su ejecución.

4. ¿Qué protege el GIL? ¿Evita las condiciones de carrera en los datos del programa?

R. El GIL protege la estructura interna del intérprete de CPython, evitando que múltiples hilos ejecuten código de Python al mismo tiempo en un mismo proceso. Impide problemas al contar y administrar las referencias de los objetos en la memoria.

5. ¿Por qué 4 hilos que duermen 1 s cada uno tardan ~1 s en total y no ~0,25 s?

R. Porque time.sleep() libera el GIL durante la espera, los 4 hilos realizan la espera de forma concurrrente en lugar de secuencial, todos inician la espera de 1 segundo al mismo tiempo, no esperan un cuarto de segundo.

6. Al crear un hilo por herencia, ¿qué método se sobrescribe y cuál nunca? ¿Por qué?

R. Se sobreescribe el run() y no se sobreescribe sobreescribir start(),por que start() es quien arranca el hilo y llama a run() internamente. Nunca se puede el init sin llamar el super().init(...).

7. ¿Qué pasa con un hilo daemon cuando termina el hilo principal? ¿Qué **no** se ejecuta?

R. El hilo daemon se muere bruscamente de inmediato cuando termina el hilo principal. No se ejecutan los bloques finally ni las tareas de limpieza pendientes.

8. Completar la regla del curso: *hilos para , procesos para*.

R. Hilos para E/S (esperar red, archivos, APIs) porque ahorran más memoria; procesos para CPU (calcular, procesamiento pesado), permiten que si un proceso falle no se afecten los demás.
